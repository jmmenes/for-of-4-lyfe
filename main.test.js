...
// HINT: this is a reduce problem, reducing a collection of values to one value.
// In this case, a collection of numbers to one total sum.
...
// makeFriendly - given a paragraph, returns a paragraph where each sentence ends with
// an exclamation point instead of the period it held before. HINT: this is a string-building
// problem, and you'll want to mostly append to the string you'll return each character,
// except when you hit a period.
...
// `cubeAll` - given an array of numbers, returns an array where each number has been
// cubed (a.k.a. raised to the third power).
...
/* `addNoises` - given an array of strings, returns an array where each string has appended 
to it that animal's noise. See tests for examples! **HINT:** You're definitely going to 
have to check for each tested-for animal name, but you could use a compound boolean in 
your `if` check, or set up an array of your own animals and use `.includes`, or use a 
`switch` statement. Either way, you can look at the tests to see which names should 
be considered which animals! */
function addNoises(array) {
  // create empty array
  let output = [];

  for (const animalName of array) {
    // Dogs
    if (
      animalName === "Fido" ||
      animalName === "Rolph" ||
...
/* `womenOnly` - given an array of strings with names and gender, returns an array of only those
 strings where the gender (the last character in the string) is 'F'. **HINT:** you can check the 
character at last index of the string to see if it's an `F`, but there's also a string method 
 that tells you if a string "ends with" another string. [Duck it!](https://duckduckgo.com/) */
...
/* `integersOnly` - given an array of numbers, returns an array of only those numbers that 
are integers. **HINT:** you can use modulus for this, it's just SEEMS weird to do so. */
